...
class BasicOptimiser(object):
    def __init__(
        ...
        **kwargs
        ...
        warn(
            FutureWarning(
                "genetic optimisers run, and work, but convergence is presently difficult. Use at your own risk."
            )
        )
        ...
        self._bounds = (kwargs.get("lower_bound") or -5, kwargs.get("upper_bound") or 5)

    # returns the genomes sorted by their losses
    def __sort_genomes(
        self, X: X_Data, y: Sequence, population: Sequence[Genome]
    ) -> List[Genome]:
        losses = {
            self._loss(X, y, genome.genome): genome for genome in population
        }  # create a dict with loss:genome
        losses = [
            losses[key] for key in sorted(list(losses.keys()))
        ]  # get the genomes from the losses sorted by key
        return losses
    ...
    def optimise(
        ...
        y: Sequence,
        child_loss: Loss = "mae",
        child_max_loss: float = 0.1,
        mutation_rate: float = 0.1,
        mutation_magnitude: float = 1,
        **kwargs
    ) -> Genome:
        ...
        """
        sort_by_loss = (
            kwargs.get("sort_by_loss")
            if kwargs.get("sort_by_loss") is not None
            else True
        )  # get the

        population = [
            Genome(uniform(self._bounds[0], self._bounds[1], size=self._genome_shape))
        ...
        return population[
            0
        ]  # Returns the best genome (there should only be 1 element by this point)
