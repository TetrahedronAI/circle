...
class TestGenome(unittest.TestCase):
    ...
    def test_mutate(self):
        ...
        noChanges = 0
        for i in [[randint(-10, 10) for _ in range(10)] for _ in range(TESTS)]:
            test = Genome(i)
            ...
            self.assertFalse(
                old_genome is new_genome, msg="there is an issue with the tests."
            )

            if (old_genome == new_genome).all():
                noChanges += 1
    ...
    @log_errors
    def test_child(self):
        test = child_of(Genome([[1, 2, 3]]), Genome([[1.1, 4, 2.8]]))
...
class TestBasicOptimiser(unittest.TestCase):
    ...
    def test_optimiser(self):
        ...
        opt = BasicOptimiser(
            loss_fn, 1000, genome_shape=(2, 2), lower_bound=1, upper_bound=4
        )  # using low population size to reduce test time
        _ = opt.optimise(
            [],
            [],
            "mae",
